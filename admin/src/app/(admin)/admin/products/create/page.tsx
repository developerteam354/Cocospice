...
export default function CreateProductPage() {
  ...
  // Refs for extra option inputs — avoids DOM getElementById hacks
  ...
  const { fields, append, remove } = useFieldArray({ control, name: 'extraOptions' });
  ...
  const addOption = () => {
    ...
    const price = parseFloat(optionPriceRef.current?.value ?? '');
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      await productService.create({
        ...
        extraOptions:    data.extraOptions, // already { name: string, price: number }[]
            ...
            <section className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <input
                  ...
                  min={0}
                  onKeyDown={handleOptionKeyDown}
                  className="w-28 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                />
                <button
                  type="button"
                  onClick={addOption}
                  ...
                  >
                    {fields.map((field, i) => (
                      ...
                      >
                        ...
                        <span className="text-emerald-400 font-semibold">₹{field.price}</span>
                        <button
                          type="button"
                          onClick={() => remove(i)}
